...
import com.hughswb.blog.dao.TagDao;
...
import com.hughswb.blog.enums.FileExtEnum;
import com.hughswb.blog.enums.FilePathEnum;
import com.hughswb.blog.exception.BizException;
import com.hughswb.blog.service.*;
import com.hughswb.blog.strategy.context.SearchStrategyContext;
import com.hughswb.blog.strategy.context.UploadStrategyContext;
import com.hughswb.blog.util.BeanCopyUtils;
import com.hughswb.blog.util.CommonUtils;
...
import static com.hughswb.blog.constant.CommonConst.ARTICLE_SET;
import static com.hughswb.blog.constant.CommonConst.FALSE;
import static com.hughswb.blog.constant.RedisPrefixConst.*;
import static com.hughswb.blog.enums.ArticleStatusEnum.DRAFT;
import static com.hughswb.blog.enums.ArticleStatusEnum.PUBLIC;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleDao, Article> implements ArticleService {
    ...
    @Override
    public Map<Integer,  List<Article>> classifiedStatistic() {
        return articleDao.selectList(new LambdaQueryWrapper<Article>().
                select(Article::getCategoryId)).stream().collect(Collectors.groupingBy(Article::getCategoryId));
    }
     ...
     */
    @Override
    public List<ArticleTimelineVo> articleTimeline(Integer cateGoryId) {
        String[] colorList = {"#409EFF", "#67C23A", "#E6A23C","#F56C6C","#000000"};
        List<Article> articles = articleDao.selectList(new LambdaQueryWrapper<Article>()
                .eq(Article::getCategoryId, cateGoryId).select(Article::getCreateTime
                        , Article::getArticleTitle, Article::getId).orderByAsc(Article::getCreateTime));

        List<ArticleTimelineVo> articleTimelineVos = new ArrayList<>(articles.size());

        Random rand = new Random();
        articles.forEach(a->{
            ArticleTimelineVo timelineVo = ArticleTimelineVo.builder().timestamp(a.getCreateTime())
                    .content(a.getArticleTitle())
                    .color(colorList[rand.nextInt(colorList.length)])
                    .auth("变成派大星")
                    .id(a.getId())
                    .size("large").build();
            articleTimelineVos.add(timelineVo);
        });
        return articleTimelineVos;
    }
